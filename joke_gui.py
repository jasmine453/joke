...
class JokeApp:
    ...
    def create_widgets(self):
        ...
        left_image = self.create_rounded_image("left_character.png", (150, 150))
        ...
        right_image = self.create_rounded_image("right_character.png", (150, 150))
...
def check():
    data = request.json
    jokes = load_jokes()
    joke = next((j for j in jokes if str(j['id']) == str(data['id'])), None)
    if not joke:
        return jsonify({'ok': False, 'msg': '题目不存在'})
    user_answer = data['answer'].strip()
    correct = user_answer in joke['answer'] or joke['answer'] in user_answer
    return jsonify({
        'ok': True,
        'is_correct': correct,
        'correct_answer': joke['answer'],
